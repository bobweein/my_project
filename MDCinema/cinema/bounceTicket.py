...
class Bounce_Dialog(object):
    ...
    def bounceinfo(self):
        ...
        try:
            ...
            sql = '''
            SELECT 
            m.MovieName,TB.StartTime,TB.EndTime,m.Price
            FROM 
            Movie m,(SELECT MA.MovieID,MA.StartTime,MA.EndTime FROM Ticket T ,MovieArrangement MA WHERE T.ID = MA.ID)TB
            WHERE 
            m.MovieID = TB.MovieID
            '''
            cur.execute(sql)
            result = cur.fetchone()
